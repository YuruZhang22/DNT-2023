...
import copy, math
# from function import add_dict


def acq_max(ac, gp, y_max, bounds, random_state, n_warmup=10000, topk=1):
    """
    ...
    xmaxs = x_tries[ys.argmax()]
    # xtmps = [copy.deepcopy(x_tries[ys.argmax()])]

    # xmaxs = []
    # for k in range(topk):
    #     idx = np.argmax(ys)
    #     xmaxs.append(copy.deepcopy(x_tries[idx]))
    #     ys[idx] = -math.inf
    ...
    return np.clip(xmaxs, bounds[:, 0], bounds[:, 1])
...
class UtilityFunction(object):
    ...
    def __init__(self, kind, kappa, xi, dim, delta=0.1, kappa_decay=1, kappa_decay_delay=0):
    ...
    @staticmethod
    def _ucb(x, gp, kappa, pre_param=None):
    ...
    @staticmethod
    def _gpucb(x, gp, dim, delta, pre_param=None):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            mean, std = gp.predict(x, return_std=True)
    ...
    @staticmethod
    def _ei(x, gp, y_max, xi, pre_param=None):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            mean, std = gp.predict(x, return_std=True) # TODO generate s + s_a here
    ...
    @staticmethod
    def _poi(x, gp, y_max, xi, pre_param=None):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            mean, std = gp.predict(x, return_std=True)
